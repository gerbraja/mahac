...
def register(data: RegisterData, db: Session = Depends(get_db)):
    ...
    if referer:
        ...
        new_user.referred_by = referer.name  # Use name instead of username
...
def verify_referral_code(referral_code: str, db: Session = Depends(get_db)):
    """Verify if a referral code is valid and return the referrer's name."""
    user = db.query(UserModel).filter(UserModel.referral_code == referral_code).first()
    if user:
        return {
            "valid": True,
