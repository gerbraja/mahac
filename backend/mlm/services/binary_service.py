...
def process_arrival_bonuses(db: Session, new_member: BinaryGlobalMember):
    """Traverse upline and pay arrival bonuses according to rules."""
    current = new_member
    level_up = 1
    ...
    while current.upline_id and level_up <= 21:
        ...
        bonus_amount = ARRIVAL_BONUS_RULES.get(level_up, 0.0)
...
def activate_binary_global(db: Session, user_id: int):
    """Activate a pre-registered user."""
    ...
    process_arrival_bonuses(db, member)
